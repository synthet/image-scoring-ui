...
export function phaseStatusColor(status) {
    ...
    if (s === 'done' || s === 'completed')
        return PHASE_STATUS_COLORS.done;
    if (s === 'running')
        return PHASE_STATUS_COLORS.running;
    if (s === 'failed')
        return PHASE_STATUS_COLORS.failed;
    if (s === 'canceled' || s === 'cancelled')
        return PHASE_STATUS_COLORS.canceled;
    if (s === 'paused')
        return PHASE_STATUS_COLORS.paused;
    if (s === 'pending' || s === 'queued')
        return PHASE_STATUS_COLORS.queued;
    if (s === 'skipped')
        return PHASE_STATUS_COLORS.skipped;
